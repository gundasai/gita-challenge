...
export default function WaitingPage() {
    const { user, userRole, userData, logout, loading } = useAuth();
    ...
    }, [startDate, router, userRole, userData?.status]);

    // Admin bypass - they can access admin panel anytime
    useEffect(() => {
        if (userRole === "admin" && hasStarted && userData?.status !== 'pending') {
            router.push("/admin");
        }
    }, [userRole, hasStarted, router, userData?.status]);
                ...
                <div className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#0C0C0C]/90 shadow-[0_40px_100px_rgba(0,0,0,0.8)] backdrop-blur-3xl">
                    ...
                    <div className="relative bg-gradient-to-b from-white/[0.05] to-transparent p-10 pb-6 text-center">
                        ...
                        {userData?.status === 'pending' ? (
                            <>
                                <span className="inline-block text-[10px] font-black uppercase tracking-[0.4em] text-yellow-500 mb-6">
                                ...
                                <p className="text-sm font-medium text-gray-400">
                                    Waiting for <span className="text-[var(--cream)]">{userData?.company || "Institution"}</span> Admin
                                </p>
                            </>
                        ) : (
                            <>
                    ...
                    <div className="p-10 space-y-10">
                        <div className="flex justify-between items-end border-b border-white/5 pb-8">
                            ...
                            <div className="space-y-1 text-right">
                                ...
                                <p className={`text-sm font-black uppercase ${userData?.status === 'pending' ? 'text-yellow-500' : 'text-green-500'}`}>
                                    {userData?.status === 'pending' ? 'Pending' : 'Registered'}
                                </p>
                            </div>
                        </div>
                            ...
                            {userData?.status === 'pending' ? (
                                <div className="text-center space-y-4">
                                    <div className="flex justify-center">
                                        <div className="h-16 w-16 md:h-20 md:w-20 bg-yellow-500/10 rounded-full flex items-center justify-center border border-yellow-500/20 animate-pulse">
                                            <Clock className="h-8 w-8 md:h-10 md:w-10 text-yellow-500" />
                                        </div>
                                    </div>
                                    <p className="text-sm text-gray-300 leading-relaxed">
                                        Your registration has been submitted successfully using the Institution Portal.
                                    </p>
                                    <p className="text-sm text-gray-300 leading-relaxed">
                                        Please wait while your institution administrator validates and approves your account. You will get access once approved.
                                    </p>

                                    <button
                                        onClick={() => logout()}
                                        className="mt-4 px-6 py-2 rounded-full border border-white/10 bg-white/5 text-sm font-medium text-white hover:bg-white/10 transition-colors"
                                    >
                                        Logout
                                    </button>
                                </div>
                        ...
                        {/* WhatsApp CTA - Only for non-pending users */}
                        {userData?.status !== 'pending' && (
